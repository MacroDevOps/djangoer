...
class ListAdminView(ModelAdminView):
    ...
    def block_pagination(self, context, nodes, page_type='normal'):
        """
        Generates the series of links to the pages.md in a paginated list.
        """
        ...
        if not pagination_required:
            ...
            # If there are 10 or fewer pages.md, display links to every page.
            # Otherwise, do some fancy
            if paginator.num_pages <= 10:
                page_range = range(paginator.num_pages)
            else:
                # Insert "smart" pagination links, so that there are always ON_ENDS
                # links at either end of the list of pages.md, and there are always
